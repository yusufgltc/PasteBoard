...
final class ClipboardStoreTests: XCTestCase {
    ...
    func testAdd_doesNotDeduplicate_images() {
        let img1 = ClipboardItem.image(fileName: "a.png")
        let img2 = ClipboardItem.image(fileName: "a.png")
        store.add(img1)
        store.add(img2)
        XCTAssertEqual(store.items.count, 2, "images are never considered duplicates")
    }
