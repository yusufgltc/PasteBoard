...
final class ClipboardStore: ObservableObject {
    ...
    func add(_ item: ClipboardItem) {
        if let existing = items.first(where: { isDuplicate($0, item) }) {
            ...
            // Skip file deletion when the new item references the same image file
            // (same content → same SHA256 filename) to avoid deleting a file still in use.
            if existing.imageFileName != item.imageFileName {
    ...
    private func isDuplicate(_ existing: ClipboardItem, _ incoming: ClipboardItem) -> Bool {
        ...
        case .image: return existing.imageFileName == incoming.imageFileName
        }
    }
